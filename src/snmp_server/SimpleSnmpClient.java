...
public class SimpleSnmpClient {
	...
	public String getAsString(OID oid) throws IOException {
       ...
       public void getAsStrings(PDU pdu) throws IOException {
		ResponseEvent event = get(pdu);
		 ...
		 System.out.println(event.getResponse().get(i).getVariable().toString());
                 System.out.println(event.getRequest().get(i).getVariable().toString());
                }
	}
	
         public void getAsString2(OID oid) throws IOException {
                 ResponseEvent event = getBULK(new OID[]{oid});
	...
	private PDU getPDU(OID oids[]) {
        ...
        private PDU getPDUBULK(OID oids[]) {
		PDU pdu = new PDU();
		for (OID oid : oids) {
			pdu.add(new VariableBinding(oid));
		}
	...
	public ResponseEvent get(OID oids[]) throws IOException {
	   ...
	   throw new RuntimeException("GET timed out");	  
        ...
        public ResponseEvent getBULK(OID oids[]) throws IOException {
	   ResponseEvent event = snmp.send(getPDUBULK(oids), getTarget(), null);
	   if(event != null) {
		   return event;
	   }
	   throw new RuntimeException("GET timed out");	  
